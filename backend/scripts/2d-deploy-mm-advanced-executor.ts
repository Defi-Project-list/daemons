...
const aavePriceOracleAddress = '0xA840C768f7143495790eC8dc2D5f32B71B6Dc113'; //?
...
async function main() {
  ...
  // Aave Advanced Script Executor
  const mmAdvancedScriptExecutorContract = await ethers.getContractFactory("MmAdvancedScriptExecutor");
  const mmAdvancedScriptExecutor = await mmAdvancedScriptExecutorContract.deploy();
  await mmAdvancedScriptExecutor.deployed();
  console.log(`mmAdvancedScriptExecutor deployed to: ${mmAdvancedScriptExecutor.address}`);
