...
describe("Vesting contract", function () {
  ...
  async function initializeVestingContract(start: number, duration: number): Promise<Contract> {
  ...
  it('allows adding beneficiaries', async () => {
    const vestingContract = await initializeVestingContract(tomorrow(), oneDay());
    await fooToken.mint(vestingContract.address, ethers.utils.parseEther("200"));

    // no error means it all went through as expected!
    await vestingContract.addBeneficiary(beneficiary1.address, ethers.utils.parseEther("100"));
    await vestingContract.addBeneficiary(beneficiary2.address, ethers.utils.parseEther("100"));
  });
  ...
  it('throws an error if trying to give to a beneficiary an amount higher than the contract balance', async () => {
    ...
    await expect(vestingContract.addBeneficiary(beneficiary1.address, ethers.utils.parseEther("201"))).to.be.revertedWith('Amount is higher than available for vesting');
  });
  ...
  it('throws an error if trying to add the same beneficiary twice', async () => {
    ...
    await vestingContract.addBeneficiary(beneficiary1.address, ethers.utils.parseEther("50"));
    await expect(vestingContract.addBeneficiary(beneficiary1.address, ethers.utils.parseEther("50"))).to.be.revertedWith('Beneficiary is already in use');
  });
  ...
  it('throws an error if trying to add beneficiaries after the start date', async () => {
    ...
    await expect(vestingContract.addBeneficiary(beneficiary1.address, ethers.utils.parseEther("50"))).to.be.revertedWith('Vesting started. Modifications forbidden');
  });
  ...
  describe('Unallocated tokens', () => {
    ...
    it('keeps track of unallocated tokens', async () => {
      const vestingContract = await initializeVestingContract(tomorrow(), oneDay());
      await fooToken.mint(vestingContract.address, ethers.utils.parseEther("200"));
      expect((await vestingContract.unallocatedTokens()).toString()).to.equal(ethers.utils.parseEther("200").toString());

      await vestingContract.addBeneficiary(beneficiary1.address, ethers.utils.parseEther("125"));
      expect((await vestingContract.unallocatedTokens()).toString()).to.equal(ethers.utils.parseEther("75").toString());

      await vestingContract.addBeneficiary(beneficiary2.address, ethers.utils.parseEther("73"));
      expect((await vestingContract.unallocatedTokens()).toString()).to.equal(ethers.utils.parseEther("2").toString());
    });
    ...
    it('allows the owner to claim unallocated tokens', async () => {
      ...
      await vestingContract.addBeneficiary(beneficiary1.address, ethers.utils.parseEther("100"));
      await vestingContract.addBeneficiary(beneficiary2.address, ethers.utils.parseEther("25"));
      expect((await vestingContract.unallocatedTokens()).toString()).to.equal(ethers.utils.parseEther("75").toString());
      ...
      expect((await vestingContract.unallocatedTokens()).toNumber()).to.equal(0);
      expect((await fooToken.balanceOf(owner.address)).toString()).to.equal(ethers.utils.parseEther("75").toString());
    });
    ...
    it('throws an error if trying to claim back unassigned tokens before the start date', async () => {
      ...
      await expect(vestingContract.claimUnallocatedTokens()).to.be.revertedWith('Vesting has not started yet');
    });
  ...
  describe('Vesting calculation', () => {
    ...
    it('at creation no token is vested', async () => {
      ...
      expect(await vestingContract.vestedAmount(beneficiary1.address)).to.be.equal(0);
    });
    ...
    it('just after the start date, tokens start getting vested', async () => {
      ...
      await network.provider.send("evm_mine");
      expect((await vestingContract.vestedAmount(beneficiary1.address)).toNumber()).to.be.almost.equal(1);
    });
    ...
    it('after the end date, all tokens are vested', async () => {
      ...
      expect(await vestingContract.vestedAmount(beneficiary1.address)).to.be.equal(200000);
    });
    ...
    it('tokens are vested proportionally to the beneficiary balance', async () => {
      ...
      expect(await vestingContract.vestedAmount(beneficiary1.address)).to.be.equal(85000);
      expect(await vestingContract.vestedAmount(beneficiary2.address)).to.be.equal(15000);
    });
  });
  ...
  it('releases to the beneficiary its due amount', async () => {
    ...
    expect(await vestingContract.releasableAmount(beneficiary1.address)).to.be.equal(0);

    // half way through it should be 100000
    await ethers.provider.send("evm_mine", [now() + oneDay() * 2 + 1]); // half way through the vesting
    expect((await vestingContract.releasableAmount(beneficiary1.address)).toNumber()).to.be.almost.equal(100000);
    ...
    expect((await fooToken.balanceOf(beneficiary1.address)).toNumber()).to.almost.equal(100000);
    expect((await vestingContract.releasableAmount(beneficiary1.address)).toNumber()).to.be.equal(0);
    ...
    await vestingContract.release(beneficiary1.address);
    expect((await fooToken.balanceOf(beneficiary1.address)).toNumber()).to.equal(200000);
    expect((await vestingContract.releasableAmount(beneficiary1.address)).toNumber()).to.be.equal(0);
  });
  ...
  it('throws an error if nothing can be released', async () => {
    ...
    expect(await vestingContract.releasableAmount(beneficiary1.address)).to.be.equal(0);
    await expect(vestingContract.release(beneficiary1.address)).to.be.revertedWith("Nothing to release");
  });
