...
describe("Treasury", function () {
  ...
  describe("Owner controlled setters", function () {
    it("can change commission percentage", async () => {
      ...
      // anyone else cannot
      await expect(
        treasury.connect(user1).setCommissionPercentage(150)
    ...
    it("can change protocol owned liquidity (POL) percentage", async () => {
      ...
      // anyone else cannot
      await expect(
        treasury.connect(user1).setPolPercentage(2500)
      ).to.be.revertedWith("Ownable: caller is not the owner");
    });
    ...
    it("throws an error if trying to set redistribution interval outside bounds", async () => {
      ...
      await expect(
        treasury.setRedistributionInterval(63158400)
      ).to.be.revertedWith("RI must be at most 730 days");

      // too low redistribution interval (setting 29 days)
      await expect(
        treasury.setRedistributionInterval(2505600)
      ).to.be.revertedWith("RI must be at least 30 days");
    });
  });
  ...
  describe("Rewards payouts requests", function () {
    it("only gas tank can initialize payout requests", async () => {
      ...
      await expect(
        treasury.requestPayout(user1.address, { value: amount })
    ...
    it("payout causes tokens to be sent to the user", async () => {
      ...
      await treasury
        .connect(gasTank)
        .requestPayout(user1.address, { value: amount });
    ...
    it("payout causes ETH to be distributed into pools accordingly", async () => {
      ...
      expect(await treasury.commissionsPool()).to.equal(
        ethers.utils.parseEther("0.01")
      );
      expect(await treasury.polPool()).to.equal(
        ethers.utils.parseEther("0.49")
      );
      expect(await treasury.redistributionPool()).to.equal(
  ...
  describe("Staking payouts requests", function () {
    it("only gas tank can initialize staking payout requests", async () => {
      const amount = ethers.utils.parseEther("1.0");
      await expect(
        treasury.stakePayout(user1.address, { value: amount })
      ).to.be.revertedWith("Unauthorized. Only GasTank");
    });
    ...
    it("staking payout causes tokens to staked on behalf of the user", async () => {
      ...
      await treasury
        .connect(gasTank)
        .stakePayout(user1.address, { value: amount });
    ...
    it("staking payout doesn't change treasury balance", async () => {
      ...
      const initialAmountOfTokensToDistribute =
        await treasury.tokensForDistribution();
      ...
      const finalAmountOfTokensToDistribute =
        await treasury.tokensForDistribution();
  ...
  describe("Staking", function () {
    ...
    it("the user cannot withdraw funds they don't own", async () => {
      const amount = ethers.utils.parseEther("1.0");
      await expect(treasury.withdraw(amount)).to.be.revertedWith(
        "Insufficient staked funds"
      );

      await expect(treasury.withdraw(0)).to.be.revertedWith(
        "Cannot withdraw 0"
      );
    });
    ...
    it("is not possible to withdraw ALL funds from the treasury", async () => {
      ...
      // withdraw all
      await expect(treasury.connect(user1).withdraw(amount)).to.be.revertedWith(
    ...
    it("reward is accumulated over time", async () => {
      ...
      expect(await treasury.redistributionPool()).to.be.equal(
      ...
      expect(await treasury.earned(user1.address)).to.be.equal(
      ...
      expect(
        earnedReward.sub(expectedReward).abs().toNumber()
      ).to.be.almost.equal(0);
    });
    ...
    it("reward zeroed after claiming", async () => {
      ...
      const earnedRewardAfterClaim: BigNumber = await treasury.earned(
        user1.address
      );
    ...
    it("rewards are accounted each time are claimed", async () => {
      ...
      expect(
        distributed.sub("2777713477338878").abs().toNumber()
      ).to.be.almost.equal(0);
    });
    ...
    it("exit function gets both reward and staked amount", async () => {
      ...
      const userStakedAmount: BigNumber = await treasury.balanceOf(
        user1.address
      );
  ...
  describe("Commissions", function () {
    it("owner can withdraw commission from the pool", async () => {
      ...
      await expect(
        treasury.connect(user1).claimCommission()
      ).to.be.revertedWith("Ownable: caller is not the owner");
      expect(await treasury.commissionsPool()).to.equal(
        ethers.utils.parseEther("0.01")
      );
  ...
  describe("Protocol Owned Liquidity", function () {
    ...
    it("LP funding uses all the ETH in the polPool + a proportional amount of DAEM", async () => {
      ...
      await treasury
        .connect(gasTank)
        .stakePayout(user1.address, { value: OneEth });
      expect(await treasury.polPool()).to.equal(
        ethers.utils.parseEther("0.49")
      );
