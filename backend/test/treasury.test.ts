...
describe("Treasury", function () {
    ...
    describe("Owner controlled setters", function () {
        ...
        it("can change redistribution interval", async () => {
            ...
            // anyone else cannot
            await expect(treasury.connect(user1).setRedistributionInterval(15552000)).to.be.revertedWith('Ownable: caller is not the owner');
        });
        ...
        it("throws an error if trying to set redistribution interval outside bounds", async () => {
            ...
            await expect(treasury.setRedistributionInterval(63158400)).to.be.revertedWith('RI must be at most 730 days');

            // too low redistribution interval (setting 29 days)
            await expect(treasury.setRedistributionInterval(2505600)).to.be.revertedWith('RI must be at least 30 days');
        });
    });
    ...
    describe("Staking payouts requests", function () {
        it("only gas tank can initialize staking payout requests", async () => {
            const amount = ethers.utils.parseEther("1.0");
            await expect(treasury.stakePayout(user1.address, { value: amount })).to.be.revertedWith('Unauthorized. Only GasTank');
        });
        ...
        it("staking payout doesn't change treasury balance", async () => {
            ...
            expect(initialAmountOfTokensToDistribute).to.not.be.equal(finalAmountOfTokensToDistribute);
        });
    });
    ...
    describe("Staking", function () {
        ...
        it("the user cannot stake funds they don't own", async () => {
            const amount = ethers.utils.parseEther("1.0");
            await expect(treasury.stake(amount)).to.be.revertedWith('ERC20: transfer amount exceeds balance');
        ...
        it('is not possible to withdraw ALL funds from the treasury', async () => {
            ...
            // withdraw all
            await expect(treasury.connect(user1).withdraw(amount)).to.be.revertedWith('Cannot withdraw all funds');
        });
        ...
        it('reward is accumulated over time', async () => {
            ...
            expect(await treasury.balanceOf(user1.address)).to.be.equal(ethers.utils.parseEther("1"));
