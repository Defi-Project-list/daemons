...
describe("Price Retriever", function () {
    ...
    it('allows adding a price feeds and retrieving data', async () => {
        ...
        await priceRetriever.addPriceFeed(btcAddress, btcPriceFeed.address, btcDecimals, feedDecimals);
    ...
    it('returns less decimals if the token decimals < feed decimals', async () => {
        ...
        await priceRetriever.addPriceFeed(usdcAddress, btcPriceFeed.address, usdcDecimals, feedDecimals);
    ...
    it('throws an exception if trying to add multiple times a feed for the same token', async () => {
        ...
        await priceRetriever.addPriceFeed(btcAddress, btcPriceFeed.address, btcDecimals, feedDecimals);
        await expect(priceRetriever.addPriceFeed(btcAddress, btcPriceFeed.address, btcDecimals, feedDecimals)).to.be.revertedWith('[PriceRetriever] Token already has a price feed');
    });
