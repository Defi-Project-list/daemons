...
import { domain, ITransferPctAction, types } from "../../shared-definitions/scripts/transfer-pct-action-messages";

describe("TransferPctScriptExecutor", function () {
    ...
    this.beforeEach(async () => {
        ...
        // Executor contract
        const TransferPctScriptExecutorContract = await ethers.getContractFactory("TransferPctScriptExecutor");
        executor = await TransferPctScriptExecutorContract.deploy();
    ...
    async function initialize(baseMessage: ITransferPctAction): Promise<ITransferPctAction> {
    ...
    it("spots a tampered message with no conditions", async () => {
        ...
        tamperedMessage.percentage = ethers.utils.parseEther("0");
    ...
    it("spots a valid message from another chain", async () => {
        let message: ITransferPctAction = JSON.parse(JSON.stringify(baseMessage));
    ...
    it('transfers the tokens', async () => {
        ...
        // check post-balance.
        expect(await fooToken.balanceOf(owner.address)).to.equal(ethers.utils.parseEther("50"));

        // the destination got his tokens
        expect(await fooToken.balanceOf(otherWallet.address)).to.equal(ethers.utils.parseEther("50"));
    });
    ...
    it("fails if the user doesn't have enough balance, even tho the balance condition was not set", async () => {
        // leaving the user at 0 balance
        fooToken.transfer(otherWallet.address, ethers.utils.parseEther("100"));
