...
describe("GasTank", function () {
  ...
  describe("Rewards and Claims", function () {
    it("allows executors to add rewards", async () => {
      ...
      await gasTank
        .connect(owner)
        .addReward(rewardAmount, user1.address, user2.address);

      // balances should have been updated
      expect(await gasTank.claimable(user2.address)).to.equal(rewardAmount);
      expect(await gasTank.balanceOf(user1.address)).to.equal(
        oneEth.sub(rewardAmount)
      );
    });
    ...
    it("when staking reward, ETH will be sent to treasury", async () => {
      ...
      const expectedReward = rewardAmount;
      expect(await fooToken.balanceOf(user2.address)).to.equal(
        BigNumber.from(0)
      );
