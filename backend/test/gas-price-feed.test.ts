...
describe("GasPriceFeed", function () {
    ...
    it("last gas price is publicly available", async () => {
        expect(await gasPriceFeed.lastGasPrice()).to.equal(1);
    });

    it("owner can update last gas price", async () => {
        await gasPriceFeed.setGasPrice(5);

        expect(await gasPriceFeed.lastGasPrice()).to.equal(5);
    });

    it("throws an error if trying to set an invalid value", async () => {
        await expect(gasPriceFeed.setGasPrice(0))
            .to.be.revertedWith('GasPrice must be greater than 0');
    });

    it("throws an error if anyone but the owner tries to update the gas price", async () => {
        await expect(gasPriceFeed.connect(otherUser).setGasPrice(1500))
